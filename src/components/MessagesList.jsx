...
  const stopMessage = useAITeacher((state) => state.stopMessage); // <-- Added stopMessage
  const { currentMessage } = useAITeacher();
  
  // NOTE: Keeping these variables, but they are now irrelevant for the English-only output
...
  // We only render the English text now.
  const renderText = (text) => (
    // Ensure text is available before rendering
...
  // --- 2. REMOVED RENDER JAPANESE ---
  // The renderJapanese function (and renderEnglish) are no longer here.
...
              {/* ⭐ CRITICAL FIX: Use the raw text from the AI response */}
              {renderText(message.answer.text)} 
            </div>
...
          {/* ⭐ REMOVED GRAMMAR BREAKDOWN SECTION */}
        </div>
      ))}
    </div>
  );
